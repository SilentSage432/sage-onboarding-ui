...
import { Octagon, AlertCircle, CheckCircle2, XCircle } from "lucide-react";
import { useArcStatus } from "@/lib/console/useArcStatus";

export default function ArcOmegaPanel() {
  const { data, loading, error, unavailable } = useArcStatus("omega");

  if (loading) {
    return (
      <div className="p-6 space-y-6">
        <h1 className="text-2xl text-white font-bold">Arc Omega</h1>
        <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-xl p-6">
          <div className="flex items-center gap-3 text-gray-400">
            <Octagon className="h-5 w-5 animate-pulse" />
      ...
      <div className="p-6 space-y-6">
        ...
        >
          ...
          <p className="text-gray-400 text-sm mt-2">
            Arc Omega status information is not currently available. This is an observational panel and will display data when the service is connected.
          </p>
        </motion.div>
      </div>
    );
  ...
  const statusIcon =
    data?.status === "active" ? (
      ...
      <Octagon className="h-6 w-6 text-gray-400" />
    );
    ...
    <div className="p-6 space-y-6">
      ...
      >
        <div className="flex items-center gap-3">
          <Octagon className="h-8 w-8 text-orange-400" />
          <div>
            <h2 className="text-white font-semibold text-lg">Status</h2>
            <p className="text-gray-400 text-sm">Arc Omega operational state</p>
          </div>
        </div>
